package white.ball.data.local_storage.room.entity.additional

import androidx.room.Entity
import androidx.room.ForeignKey
import androidx.room.ForeignKey.Companion.CASCADE
import androidx.room.PrimaryKey
import white.ball.data.local_storage.room.entity.NoteDTO
import white.ball.domain.model.additional.TaskByNoteDomainModel

@Entity (
    foreignKeys = [
        ForeignKey(
            entity = NoteDTO::class,
            parentColumns = ["noteId"],
            childColumns = ["noteId"],
            onDelete = CASCADE
        )
    ]
)
data class TaskByNoteDTO(
    @PrimaryKey(autoGenerate = true)
    val taskId: Long,
    var title: String,
    var isDone: Boolean,
    val noteId: Long,
)

fun TaskByNoteDTO.toTaskDomainModel() = TaskByNoteDomainModel (
    taskId = this.taskId,
    title = this.title,
    isDone = this.isDone,
    noteId = this.noteId,
)

fun TaskByNoteDomainModel.toTaskDTO() = TaskByNoteDTO (
    taskId = this.taskId,
    title = this.title,
    isDone = this.isDone,
    noteId = this.noteId,
)

fun TaskByNoteDomainModel.toTaskDTO(noteId: Long) = TaskByNoteDTO(
    taskId = this.taskId,
    title = this.title,
    isDone = this.isDone,
    noteId = noteId,
)


