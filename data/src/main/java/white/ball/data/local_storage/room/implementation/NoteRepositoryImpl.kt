...
import white.ball.data.local_storage.room.entity.additional.toTaskDTO
import white.ball.data.local_storage.room.entity.additional.toTaskDomainModel
import white.ball.data.local_storage.room.entity.agregate.toNoteDomainModel
import white.ball.data.local_storage.room.entity.toNoteDTO
...
class NoteRepositoryImpl @Inject constructor(
    ...
    override suspend fun addNote(noteModelUI: NoteDomainModel): Long {
        ...
        val tasksDTO: List<TaskByNoteDTO> = noteModelUI.taskList.map { task: TaskByNoteDomainModel -> task.toTaskDTO(newNoteId) }
    ...
    override suspend fun editNote(noteModelUI: NoteDomainModel) {
        ...
        val tasksDTO: List<TaskByNoteDTO> = noteModelUI.taskList.map { task: TaskByNoteDomainModel -> task.toTaskDTO(noteModelUI.noteId) }
        noteDao.insertTaskList(tasksDTO)
    }
    ...
    override fun getNoteWithTasksById(noteId: Long): Flow<NoteDomainModel> {
        return noteDao.getNoteWithTasksById(noteId).map { it.toNoteDomainModel() }
    }
