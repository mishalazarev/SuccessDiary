package white.ball.domain.use_case.note

import kotlinx.coroutines.test.runTest
import org.junit.jupiter.api.AfterEach
import org.junit.jupiter.api.Test
import org.mockito.Mockito
import org.mockito.Mockito.mock
import white.ball.domain.model.additional.TaskByNoteDomainModel
import white.ball.domain.repository.NoteRepository

class InsertTaskUseCaseTest {

    private val noteRepository = mock(NoteRepository::class.java)
    private val insertTaskUseCase = InsertTaskUseCase(noteRepository)

    @AfterEach
    fun tearDown() {
        Mockito.reset(noteRepository)
    }

    @Test
    fun `insert task which belong to note`() = runTest {
        val testData = TaskByNoteDomainModel(
            noteId = 0,
            title = "",
            localId = "",
            isDone = false
        )

        insertTaskUseCase(taskByNoteDomainModel = testData)

        Mockito.verify(noteRepository, Mockito.times(1))
            .insertTask(testData)
    }
}